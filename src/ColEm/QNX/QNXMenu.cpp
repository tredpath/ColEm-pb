...
void UpdateRomList()
{
	...
	dirp = opendir("shared/misc/ColEm/ROM/");
	if (dirp != NULL)
	{
		for (;;)
		{
...
int AutoLoadRom(char* romfile, bool showAbout)
{
	int status = 0;
	int M = 0;
	...
	size_t count = 0;
	if (showAbout)
		list = (const char**)malloc((sortedvecList.size() + 3) * sizeof(char*));
	else
		list = (const char**)malloc(sortedvecList.size() * sizeof(char*));

	memset(romfile, 0x0, sizeof(romfile));
				...
				if (strcmp(label, DIALOG_OK_LABEL) == 0)
				{
					...
					if (num != 0)
					{
						if (*response[0] < sortedvecList.size())
						{
							printf("%s\n", list[*response[0]]);
							fflush(stdout);
							strcpy(romfile, list[*response[0]]);
							M = 1;
						}
						else if (*response[0] == (sortedvecList.size() + 1))
						{
							free(list);
							return 2;
						}
...
void ProgramInfo()
{
	...
	if (dialog_create_alert(&alert_dialog) != BPS_SUCCESS)
		fprintf(stderr, "Failed to create alert dialog.");
	dialog_set_title_text(alert_dialog, "ColEm-pb 1.0");
	if (dialog_set_alert_html_message_text(alert_dialog, "<b>Original Source:</b> Marat Fayzullin<br><b>Website:</b> fms.komkon.org/ColEm/") != BPS_SUCCESS)
	{
		fprintf(stderr, "Failed to set alert dialog message text.");
		dialog_destroy(alert_dialog);
		alert_dialog = 0;
...
int ShowOptionMenu()
{
	...
	const char* list[7] = { "Toggle Options", "Auto Fire R", "Auto Fire L", "Show TV Lines", "Soften", "", "Reset" };
	size_t count = 7;
