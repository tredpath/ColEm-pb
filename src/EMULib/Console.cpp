...
#if !defined(_MSC_VER) && !defined(__WATCOMC__) && !defined(__PLAYBOOK__)
...
int CONMenu(int X,int Y,int W,int H,pixel FGColor,pixel BGColor,const char *Items,int Item)
{
  ...
  do
  {
    ...
    /* If no mouse buttons clicked, get key */
    if(!(J&MSE_BUTTONS)) J=GetKey();
...
const char *CONFile(pixel FGColor,pixel BGColor,const char *Ext)
{
  ...
  do
  {
    ...
    if(!(D=opendir("shared/misc/ColEm/ROM"))) break;
