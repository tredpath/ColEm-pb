...
#ifdef MAEMO
#define KEYSTEP     34                   /* Key step in pixels      */
#define KEYSIZE     30                   /* Key size in pixels      */
#else
...
#ifdef MAEMO
static const int KBDOffsets[YKEYS] = { 0,0,10,-10,20,10 };
#else
...
void SetPenCues(unsigned int Buttons,const char *CueText)
{
  ...
  for(J=0;J<sizeof(PenCues)/sizeof(PenCues[0]);++J)
    if(Buttons&(1<<J)) PenCues[J]=CueText;
}
...
void DrawPenCues(Image *Img,int ShowDialpad,pixel Color)
{
  ...
  /* Button labels */
  X=W-W3+2;
  if(PenCues[6]) PrintXY(Img,PenCues[6],2,2,Color,-1);
  if(PenCues[7]) PrintXY(Img,PenCues[7],X,2,Color,-1);
  if(PenCues[5]) PrintXY(Img,PenCues[5],X,(H>>4)+2,Color,-1);
  if(PenCues[4]&&PenCues[5])
  {
    Y=(H>>1)-(H>>4)+2;
    J=X+(strlen(PenCues[4])<<3);
    PrintXY(Img,PenCues[4],X,Y,Color,-1);
    PrintXY(Img,"+",J,Y,Color,-1);
    PrintXY(Img,PenCues[5],J+8,Y,Color,-1);
  }
  if(PenCues[4]) PrintXY(Img,PenCues[4],X,(H>>1)+(H>>4)+2,Color,-1);
  if(PenCues[9]) PrintXY(Img,PenCues[9],2,H-(H>>4)+2,Color,-1);
  if(PenCues[8]) PrintXY(Img,PenCues[8],X,H-(H>>4)+2,Color,-1);
