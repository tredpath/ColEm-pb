...
#include "../Sound.h"
#include "../Console.h"
#include "../../ColEm/Coleco.h"
#include "../../ColEm/QNX/QNXMenu.h"
...
static int Effects    = EFF_SAVECPU; /* EFF_* bits */
...
int InitQNX(const char *Title,int Width,int Height)
{
...
#ifdef USE_SDL
  int flags = SDL_INIT_VIDEO | SDL_INIT_AUDIO | SDL_INIT_TIMER | SDL_INIT_NOPARACHUTE;
  ...
  if (!(gui = SDL_SetVideoMode(WIDTH, HEIGHT, 32, flags)))
  {
...
int ShowVideo(void)
{
  ...
  {
#ifdef USE_SDL
	  SDL_BlitSurface(VideoImg->blitSurface, NULL, gui, NULL);
	  SDL_Flip(gui);
	  //SDL_Delay(2000);
#else
...
#ifdef USE_SDL
	if (gui->flags & SDL_SRCCOLORKEY)
		Img->blitSurface = SDL_CreateRGBSurface(SDL_SWSURFACE, WIDTH, HEIGHT, gui->format->BitsPerPixel, gui->format->Rmask, gui->format->Bmask, gui->format->Gmask, gui->format->Amask);
	else
		Img->blitSurface = SDL_CreateRGBSurface(SDL_SWSURFACE, WIDTH, HEIGHT, gui->format->BitsPerPixel, gui->format->Rmask, gui->format->Bmask, gui->format->Gmask, 0);
	Img->Data = (Uint32*)Img->blitSurface->pixels;
	Img->D = Img->blitSurface->format->BitsPerPixel;
	Img->W = Img->blitSurface->w;
	Img->H = Img->blitSurface->h;
	Img->stride = Img->blitSurface->pitch;
	Img->L = Img->stride * 8.0 / Img->D;
#else
...
{
#ifdef USE_SDL
	SDL_FreeSurface(Img->blitSurface);
#else
...
void handleButtonPress(int keysym, bool pressed)
{
	if (pressed)
	{
		...
		case SDLK_LEFT:
		case KEYCODE_LEFT:         JoyState|=BTN_LEFT;JoyState&=~BTN_RIGHT;LastKey=CON_LEFT;break;
		case SDLK_RIGHT:
		case KEYCODE_RIGHT:        JoyState|=BTN_RIGHT;JoyState&=~BTN_LEFT;LastKey=CON_RIGHT;break;
		case SDLK_UP:
		case KEYCODE_UP:           JoyState|=BTN_UP;JoyState&=~BTN_DOWN;LastKey=CON_UP;break;
		case SDLK_DOWN:
		case KEYCODE_DOWN:         JoyState|=BTN_DOWN;JoyState&=~BTN_UP;LastKey=CON_DOWN;break;
		case KEYCODE_Q:
		case KEYCODE_E:
		case KEYCODE_T:
		case KEYCODE_U:
		...
		/* Call user handler */
		if(keysym&&KeyHandler) (*KeyHandler)(keysym|KeyModes);
	}
	else
	{
		...
		/* Call user handler */
		if(keysym&&KeyHandler) (*KeyHandler)(keysym|CON_RELEASE|KeyModes);
	}
}
...
void handleScreenEvent(bps_event_t* event)
{
	...
	case SCREEN_EVENT_KEYBOARD:
		...
		handleButtonPress(J, type & KEY_DOWN ? 1 : 0);
		break;
...
void SDLProcessEvents()
{
	...
	while (SDL_PollEvent(&event))
	{
		...
		case SDL_QUIT:
			...
			break;
		case SDL_KEYUP:
			handleButtonPress(event.key.keysym.sym, false);
			break;
		case SDL_KEYDOWN:
			handleButtonPress(event.key.keysym.sym, true);
			break;
...
/** values.                                                 **/
/*************************************************************/
unsigned int BPSGetColor(unsigned char R,unsigned char G,unsigned char B)
{
  return(PIXEL(R,G,B));
}
